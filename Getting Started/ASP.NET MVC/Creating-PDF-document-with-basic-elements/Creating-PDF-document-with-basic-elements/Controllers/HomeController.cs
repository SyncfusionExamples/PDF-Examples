...
namespace Creating_PDF_document_with_basic_elements.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult GenerateInvoicePDF()
        {
            ...
            PdfImage image = PdfImage.FromFile(Server.MapPath("~/App_Data/logo.jpg"));

            graphics.DrawImage(image, new RectangleF(graphics.ClientSize.Width - 200, result.Bounds.Y, 190, 45));
            ...
            //Load the XML file. 
            FileStream xmlStream = new FileStream(Server.MapPath("~/App_Data/Products.xml"), FileMode.Open, FileAccess.Read);

            //Get products list to create invoice 
            IEnumerable<CustOrders> products = Orders.GetProducts(xmlStream);
            ...
            //Load the XML file. 
            FileStream xmlStream1 = new FileStream(Server.MapPath("~/App_Data/Products.xml"), FileMode.Open, FileAccess.Read);

            //Get the shipping address details. 
            IEnumerable<ShipDetails> shipDetails = Orders.GetShipDetails(xmlStream1);
    ...
    {
        public static IEnumerable<CustOrders> GetProducts(FileStream xmlStream)
        {
        ...
        public static IEnumerable<ShipDetails> GetShipDetails(FileStream xmlStream)
        {
            using (StreamReader reader = new StreamReader(xmlStream, true))
            {
                string data = reader.ReadToEnd();
